...
### Fixed
- **Android Play Store Package ID**: Release builds now publish under the `com.dotakeeper.dotakeeper.beta` package id so they are accepted by the Google Play app listing (previously the release build produced `com.volthawk.dota_keeper`, which Play rejected).

## [1.2.1] - 2026-06-30

### Fixed
- **Android Release Crash on Launch**: Release builds crashed immediately on launch because R8 minification stripped Tauri/wry methods called reflectively over JNI (e.g. `WryActivity.getId()`), surfacing after the Tauri 2.11 upgrade. Minification is now disabled for release builds, with keep rules added in `proguard-rules.pro` for when it is safe to re-enable.
